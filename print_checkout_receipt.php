...
$qry = $conn->query("SELECT p.*,c.name as cname,c.rate,l.location as lname FROM parking_list p inner join category c on c.id = p.category_id inner join parking_locations l on l.id = p.location_id where p.id= ".$_GET['id']);
foreach($qry->fetch_assoc() as $k => $v){
	$$k = $v;
}
$in_qry = $conn->query("SELECT * FROM time_stamp where pl_id = '".$_GET['id']."' and status = 1");
$in_timstamp = $in_qry->num_rows > 0 ? date("Y-m-d H:i",strtotime($in_qry->fetch_array()['created_timestamp'])) : 'N/A';
$out_qry = $conn->query("SELECT * FROM time_stamp where pl_id = $id and status = 2");
...
<p><center><b> Customer Receipt</b></center></p>
