...
Class Action {
	...
	function save_vehicle(){
		...
		while($i == 1){
			$chk = $this->db->query("SELECT * from parking_list where ref_no = '$ref_no' ")->num_rows;
			if($chk <= 0)
				$i =0;
		}
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO parking_list set ".$data);
			if($save){
				$id = $this->db->insert_id;
				$park = $this->db->query("INSERT INTO time_stamp set pl_id = $id ");
			}
		}else{
			$save = $this->db->query("UPDATE parking_list set ".$data." where id=".$id);
		}
		if($save){
	...
	function delete_vehicle(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM parking_list where id = ".$id);
		if($delete){
				return 1;$this->db->query("DELETE FROM time_stamp where pl_id = ".$id);
			}
	}
	function checkout_vehicle(){
		...
		$data .=", status=2 ";
		$save = $this->db->query("INSERT INTO time_stamp set ".$data);
		if($data){
			$data = " amount_tendered = '$amount_tendered' ";
			$data .=", amount_due='$amount_due' ";
			$data .=", amount_change='$amount_change' ";
			$data .=", status=2 ";
			$this->db->query("UPDATE parking_list set $data where id = $pl_id");
			return 1;
		}
	}
}